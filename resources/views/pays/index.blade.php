...
@section('main')
    ...
    <div
        class="flex flex-wrap text-gray-600 rounded-2xl border-4 border-yellow-400 bg-yellow-100 border-dashed shadow hover:shadow-lg p-5 justify-center">
        ...
        @if ($deal->cost != $deal->total)
                    <span class="text-red-400 text-base line-through">{{number_format($deal->cost, 2, '.', ' ')}}</span>
                @endif
        <svg xmlns="http://www.w3.org/2000/svg" class="h-7 m-1 mr-3" viewBox="0 0 24 24" fill="currentColor"><path
